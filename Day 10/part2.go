...
func main() {
    ...
    for {
        ...
        for _, bot := range bots {
            ...
            // check if we have an output for the first three outputs
            if outputs["0"] != nil && outputs["1"] != nil && outputs["2"] != nil {
                fmt.Println(outputs["0"].values[0] * outputs["1"].values[0] * outputs["2"].values[0])
                return
            }
        }
    }
